...
Node-level workflow information is displayed via a content app, itself having three sub-views: active workflow, configuration and history.

- **Active workflow:** from this view, workflows can be initiated or actioned (assuming the current user has permission to do so. Plumber supports variant workflows, allowing all variants to be submitted to language-specific workflow or all into the workflow defined for the default language. The current variant can also be submitted individually - control is completely granular.
- **Configuration:** the explicit approval flow for the current node can be set from this view, for the current variant.
- **History:** provides an overview of all workflow activity for the current node, with detailed reporting.
...
- **Admin dashboard:** the default view in the Workflow section, the admin dashboard renders a chart displaying workflow activity over the selected time period (defaults to 28 days). Tasks are grouped by status: approved, cancelled, rejected and pending. The dashboard also displays a notification when the installed Plumber version needs updating.
